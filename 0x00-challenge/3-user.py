...
- password: private MD5 hash string
"""
...
if __name__ == '__main__':
    ...
    user_2.password = 89
    if user_2.password is not None:
        print("User.password should be None if\
               setter to an integer")

    if not user_1.is_valid_password(u_pwd):
        print("is_valid_password should return \
              True if it's the right password")

    if user_1.is_valid_password("Fakepwd"):
        print("is_valid_password should return False\
               if it's not the right password")
    ...
    if user_1.is_valid_password(89):
        print("is_valid_password should return\
               False if compare with integer")

    if user_2.is_valid_password("No pwd"):
        print("is_valid_password should return False \
              if no password set before")
